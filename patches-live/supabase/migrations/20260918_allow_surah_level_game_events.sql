-- Preserve surah-level analytics for games about surah order/metadata while
-- keeping review_queue strictly ayah-level. An ayah can never exist without a
-- surah, but a general question may legitimately reference only a surah.
...
create or replace function public.record_game_ayah_event(
  ...
  p_question_type text default 'unknown',
...
begin
  ...
  if p_surah_number is not null and (p_surah_number < 1 or p_surah_number > 114) then raise exception 'invalid_surah'; end if;
  if p_ayah_number is not null and p_ayah_number < 1 then raise exception 'invalid_ayah'; end if;
  if p_ayah_number is not null and p_surah_number is null then raise exception 'ayah_requires_surah'; end if;
  ...
  if v_has_ayah then
    if p_is_correct then
      update public.review_queue set
        correct_recovery_count = correct_recovery_count + 1,
        last_correct_at = now(),
      ...
      insert into public.review_queue(child_id, surah_number, ayah_number, priority, error_count, last_error_at, next_review_at, last_game_id, last_question_type)
      values(v_session.child_id, p_surah_number, p_ayah_number, 3, 1, now(), now() + interval '12 hours', v_session.game_id, v_question_type)
      on conflict(child_id, surah_number, ayah_number) do update set
        priority = least(10, public.review_queue.priority + 2),
