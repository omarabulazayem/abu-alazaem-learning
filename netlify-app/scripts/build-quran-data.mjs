...
function assignBoundary(globalIndex, boundaries, suraMap) {
  ...
  for (const boundary of boundaries) {
    if (toGlobalIndex(boundary, suraMap) <= globalIndex) current = boundary; else break;
  }
...
const suras = parseTags(xml, "sura").map(x => ({ number:integer(x.index),name:x.name,transliteration:x.tname,englishName:x.ename,revelationType:x.type,revelationOrder:integer(x.order),rukus:integer(x.rukus),ayahCount:integer(x.ayas),start:integer(x.start) }));
...
if (pages.length < 604 || juzs.length !== 30 || quarters.length !== 240) throw new Error(`QuranData metadata boundary failure: pages=${pages.length}, juzs=${juzs.length}, quarters=${quarters.length}`);
...
for (const surah of suras) {
  for (let ayah = 1; ayah <= surah.ayahCount; ayah++) {
    ...
    const rub = assignBoundary(globalIndex, quarters, suraMap);
    verses.push({ key:`${surah.number}:${ayah}`,surahNumber:surah.number,surahName:surah.name,ayahNumber:ayah,orderInSurah:ayah,globalIndex:globalIndex+1,text,page:assignBoundary(globalIndex,pages,suraMap),juz:assignBoundary(globalIndex,juzs,suraMap),hizb:rub?Math.ceil(rub/4):null,rub,words,beginning:words.slice(0,Math.min(3,words.length)).map(w=>w.text),ending:words.slice(Math.max(0,words.length-3)).map(w=>w.text) });
  }
}
const data = {schemaVersion:1,generatedAt:new Date().toISOString(),source:{text:"Tanzil Quran Text (Uthmani)",textUrl:textSource,metadata:"Tanzil Quran Metadata",metadataUrl:metadataSource,canonicalSource:"https://tanzil.net",license:"Creative Commons Attribution 3.0",rule:"Verbatim Quran text. Do not edit generated verse text.",sha256:textSha256},stats:{surahs:suras.length,ayahs:verses.length,pages:pages.length,juzs:juzs.length,hizbs:60,rubs:quarters.length},surahs:suras.map(({start,...s})=>s),verses};
