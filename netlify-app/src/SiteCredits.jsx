...
export default function SiteCredits(){
  return <details className="site-media-credits" dir="rtl">
    <summary>مصادر الصور والتراخيص</summary>
    <div className="site-media-credit-list">{credits.map(item=><span key={item.source}><a href={item.source} target="_blank" rel="noreferrer">{item.label}</a> — {item.creator} — <a href={item.licenseUrl} target="_blank" rel="noreferrer">{item.license}</a></span>)}</div>
  </details>;
}
