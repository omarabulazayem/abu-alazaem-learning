...
const classicGames=implementedGamesByGroup("classic");
...
export default function GamesHub(){
  ...
  const engineGames=[...coreGames,...expansionGames];
  return <div className="app game-shell game-shell-v2 game-world" dir="rtl">
    ...
    <main className="wrap page">
      ...
      {classicGames.length>0&&<section className="world-section classic-zone"><div className="world-heading"><div><span>ألعاب Legacy</span><h2>ألعاب تعمل بالنظام السابق</h2><p>تظل قابلة للعب حاليًا، لكنها مميزة بوضوح إلى أن يتم نقل تسجيل جلساتها إلى GameEngine في مرحلة استكمال الألعاب.</p></div></div><div className="game-grid compact-games">{classicGames.map(game=><article className="game-card" key={game.id}><div className="game-card-top"><div className="game-icon"><Icon name={game.icon} size={38}/></div><span className="level-chip">Legacy</span></div><h2>{game.title}</h2><p>{game.description}</p><button className="game-play" disabled={!learningActorReady(viewer)} onClick={()=>navigate(game.route)}>ابدأ اللعبة <Icon name="arrow" size={18}/></button></article>)}</div></section>}
