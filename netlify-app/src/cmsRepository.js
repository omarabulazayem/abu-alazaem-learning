...
export async function getContentMeta(contentId){
  if(!contentId)return [];
  return publicRest(`/cms_content_meta?content_id=eq.${encodeURIComponent(contentId)}&select=meta_key,meta_value,id&order=id.asc`);
}

export async function getMedia(mediaId){
  if(!mediaId)return null;
  const rows=await publicRest(`/cms_media?id=eq.${encodeURIComponent(mediaId)}&select=*&limit=1`);
  return rows?.[0]||null;
}

export async function getNavigation(location,{locale="ar"}={}){
  if(!location)return [];
  return publicRest(`/cms_navigation?location=eq.${encodeURIComponent(location)}&locale=eq.${encodeURIComponent(locale)}&select=*&order=sort_order.asc,created_at.asc`);
}

export async function getOption(key){
  if(!key)return null;
  const rows=await publicRest(`/cms_options?option_key=eq.${encodeURIComponent(key)}&is_public=eq.true&select=option_key,option_value&limit=1`);
  return rows?.[0]?.option_value??null;
}

export async function listTaxonomy(taxonomy,{locale="ar"}={}){
  if(!taxonomy)return [];
  return publicRest(`/cms_taxonomies?taxonomy=eq.${encodeURIComponent(taxonomy)}&select=id,taxonomy,description,parent_id,sort_order,cms_terms!inner(id,name,slug,locale)&cms_terms.locale=eq.${encodeURIComponent(locale)}&order=sort_order.asc`);
}
